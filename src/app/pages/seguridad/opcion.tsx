...
const OpcionPage = () => {
    ...
    const loadModulos = async () => {
        try {
            ...
            // Ensure unique modulos by id_modulo and filter out invalid entries
            const uniqueModulos = (data ?? [])
                ...
                .filter((modulo, index, self) => {
                    // Filter out modulos without valid id_modulo
                    if (!modulo.id_modulo || modulo.id_modulo === 0) {
                        console.warn('Modulo record missing id_modulo:', modulo);
                        return false;
                    }
                    
                    // Keep only the first occurrence of each id_modulo
    ...
    const getOpciones = async () => {
        try {
            ...
            // Ensure unique opciones by id_opcion and filter out invalid entries
            const uniqueOpciones = (data ?? []).filter((opcion, index, self) => {
                // Filter out opciones without valid id_opcion
                if (!opcion.id_opcion || opcion.id_opcion === 0) {
                    console.warn('Opcion record missing id_opcion:', opcion);
                    return false;
                }
                
                // Keep only the first occurrence of each id_opcion
                return self.findIndex(other => other.id_opcion === opcion.id_opcion) === index;
            });

            if (data && data.length !== uniqueOpciones.length) {
                console.warn(`Filtered ${data.length - uniqueOpciones.length} duplicate or invalid Opcion records`);
            }

            setOpciones(uniqueOpciones);
            setError(null);
        } catch (error) {
